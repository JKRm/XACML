...
function con(nid) {
	...
	var hi2 = document.getElementById("tr"+nid);
		if (confirm("确定删除？")) {
			$.post(getRootPath()+"/s3/deleteObjectAction.action",	
				"objectId="+nid,		
	...
	function rename(obj,oid) {
		...
		function affirm(oid){
			...
			if (confirm("确定移动？")) {
				if($("#platform").val()=='amazon'){
			$.post(getRootPath()+"/s3/moveObjectAction.action",	
			...
			"html");
				...
				else if($("#platform").val()=='local'){
					$.post(getRootPath()+"/bucket/movSL.action",	
		...
		function affirm4copy(oid){
			...
			if (confirm("确定复制？")) {
				if($("#platform").val()=='amazon'){
			$.post(getRootPath()+"/s3/copyObjectAction.action",	
				"objectId="+oid+"&tarBucket="+$("#bucket").val(),	
				function(data, statusText) {
					var realobj=eval('('+data+')');
					if(realobj.result=='success'){
			...
			"html");
					...
					$.post(getRootPath()+"/bucket/cpSL.action",	
							"objectId="+oid+"&bucketId="+$("#bucket").val(),	
							function(data, statusText) {
								var realobj=eval('('+data+')');
								if(realobj.result=='success'){
	...
	function saveName(){
		...
		if (confirm("确定重命名？")) {
			$.post(getRootPath()+"/s3/renameObjectAction.action",	
				...
				function(data, statusText) {
					...
					if(realobj.result=='success'){
						...
						obj.innerHTML="重命名";
						sAlert("","已成功将 "+realobj.oldname+" 重命名为 "+newName+" 。");
					}
					else if(realobj.result=='error'){
	...
	function getPath(oid){
		$.post(getRootPath()+"/s3/GetS3.action",	
				"objectId="+oid,		
				function(data, statusText) {
					var realobj=eval('('+data+')');
					if(realobj.result=='success'){
						window.open('../showDownload.jsp','下载地址', 'height=100, width=400, top=400, left=400, toolbar=no, menubar=no, scrollbars=no,resizable=no,location=no, status=no');
					}
					else if(realobj.result=='error'){
