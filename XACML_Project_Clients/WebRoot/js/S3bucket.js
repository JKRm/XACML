...
function con(nid) {
	...
	var hi2 = document.getElementById("tr"+nid);
		if (confirm("确定删除？")) {
			$.post(getRootPath()+"/s3/delBucketAction.action",	
				"bucketId="+nid,		
...
function saveBucket(){
	...
	if (confirm("确定添加？")) {
		$.post(getRootPath()+"/s3/addBucketAction.action",	
			"newName="+newName,		
			function(data, statusText) {
				var realobj=eval('('+data+')');
				if(realobj.result=='success'){
					...
					sAlert("","添加失败！该bucket名可能在云平台上已存在！");
					$(hi).remove();
...
function upfile(obj,oid){
	...
	if(obj.innerHTML=='上传文件▼'){
		...
		form1.id = 'form1';
		form1.method = 'post';
		form1.action = getRootPath() + '/s3/uploadObjectAction.action';
	...
	$.post(getRootPath()+"/s3/getPremAction.action",	
			"bucketId="+bid,		
			function(data, statusText) {
				var realobj=eval('('+data+')');
				if(realobj.result=='success'){
					window.location.href=getRootPath()+
					"/s3/getObjectListAction.action?bucketId="+bid;
				}
				else if(realobj.result=='deny'){
